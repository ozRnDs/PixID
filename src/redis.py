...
class RedisInterface:
    ...
    def create_vector_index(self):
        ...
        try:
            self.client.execute_command(
                ...
                "ON", "JSON",                   # Use JSON data type
                "PREFIX", "1", "embedding:",    # Key prefix for the embeddings
                "SCHEMA",
                "$.embedding", "AS", "embedding", 
                "VECTOR", "FLAT", "6",          # Vector type and arguments count
    ...
    def process_redis_results(self, results):
        ...
        for i in range(1, len(results), 2):  # Iterate over doc_id and fields pairs
            ...
            image_name = doc_parts[1]  # Extract image name (e.g., 'embedding:20230707_103839.jpg')
            face_bbox = [int(float(item)) for item in doc_parts[2].split(",")]  # Bounding box coordinates
    ...
    def vector_search(self, query_embedding: list, k: int = 5):
        ...
        try:
            results = self.client.execute_command(
                ...
                "SORTBY", "score", "ASC",
                "RETURN", "3", "image_name", "face_bbox", "score", "DIALECT","2"
            )
        except redis.exceptions.ResponseError as e:
